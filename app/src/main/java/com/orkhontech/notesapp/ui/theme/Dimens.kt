package com.orkhontech.notesapp.ui.theme

import androidx.compose.ui.unit.dp
import androidx.compose.ui.unit.sp

object Dimens {
    /*** 0.dp*/
    val spacing_xxxxs = 0.dp

    /*** 2.dp*/
    val spacing_xxxs = 2.dp

    /*** 4.dp*/
    val spacing_xxs = 4.dp

    /*** 8.dp*/
    val spacing_xs = 8.dp

    /*** 12.dp*/
    val spacing_s = 12.dp

    /*** 16.dp*/
    val spacing_m1 = 16.dp

    /*** 20.dp*/
    val spacing_m2 = 20.dp

    /*** 24.dp*/
    val spacing_l = 24.dp

    /*** 28.dp*/
    val spacing_l2 = 28.dp

    /*** 32.dp*/
    val spacing_xl = 32.dp

    /*** 48.dp*/
    val spacing_xxl = 48.dp

    /*** 2.sp*/
    val font_size_xxxs = 2.sp

    /*** 4.sp*/
    val font_size_xxs = 4.sp

    /*** 8.sp*/
    val font_size_xs = 8.sp

    /*** 10.sp*/
    val font_size_10 = 10.sp

    /*** 11.sp*/
    val font_size_11 = 11.sp

    /*** 12.sp*/
    val font_size_s = 12.sp

    /*** 13.sp*/
    val font_size_13 = 13.sp

    /*** 14.sp*/
    val font_size_s1 = 14.sp

    /*** 15.sp*/
    val font_size_15 = 15.sp

    /*** 16.sp*/
    val font_size_m1 = 16.sp

    /*** 18.sp*/
    val font_size_18 = 18.sp

    /*** 20.sp*/
    val font_size_m2 = 20.sp

    /*** 22.sp*/
    val font_size_22 = 22.sp

    /*** 24.sp*/
    val font_size_l = 24.sp

    /*** 28.sp*/
    val font_size_l1 = 28.sp

    /*** 32.sp*/
    val font_size_xl = 32.sp

    /*** 36.sp*/
    val font_size_xxl = 36.sp

    /*** 48.sp*/
    val font_size_xxxl = 48.sp

    val letterSpacing01 = 0.1.sp
    val letterSpacing02 = 0.2.sp
    val letterSpacing03 = 0.3.sp
    val letterSpacing04 = 0.4.sp
    val letterSpacing05 = 0.5.sp
    val letterSpacing08 = 0.8.sp
    val letterSpacingMinus01 = -(0.1).sp
    val letterSpacingMinus02 = -(0.2).sp
    val letterSpacingMinus03 = -(0.3).sp
    val letterSpacingMinus04 = -(0.4).sp
}